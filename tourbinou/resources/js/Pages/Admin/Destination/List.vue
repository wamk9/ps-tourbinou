...
import DestinationDelete from '@/Pages/Admin/Destination/Delete.vue';
...
<template>
    ...
    <Modal :show="showDeleteModal" :closeable="true" @close="closeDeleteModal">
        <DestinationDelete v-model:show="showDeleteModal" :id="selectedId" :name="selectedName" @updateDatabase="updateDatabase"/>
    </Modal>


    <AdminLayout id="destination-list-page">
...
export default {
    ...
    methods: {
        updateDatabase() {
            (async () => {
                const result = await Api.getAsync(route('api.admin.destination.index'));

                if (result.code == 200) {
                    this.destinationItems = result.response.message;
                    this.$refs.datatable.dt.clear().draw();
                }
            })();
        },
        goToCreatePage() {
            window.location.href = route('pages.admin.destination.store')
        },
        goToEditPage(selectedId) {
            window.location.href = route('pages.admin.destination.update', selectedId );
        },
        openDeleteModal(selectedId) {
            const destination = this.destinationItems.find((obj) => obj.id == selectedId );
            this.selectedName = !!destination ? destination.name : null;
